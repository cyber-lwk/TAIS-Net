...
class SparseCausalAttention(CrossAttention):
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None):
        batch_size, sequence_length, _ = hidden_states.shape  # (b f) seq_length query
...
if __name__ == "__main__":
    ...
    #
    # # 假设 TemporalAttention 已经定义（如上文代码所示）
    #
    # # 示例参数
    # query_dim = 256  # 每个时间步的特征维度
    # num_heads = 8
    # dim_head = 32
    # dropout = 0.1
    # video_length = 16  # 视频序列的长度（帧数）
    # batch_size = 2  # 批量大小
    #
    # # 创建 TemporalAttention 实例
    ...
    #     rotary_emb=None  # 不使用旋转位置编码
    # )
    #
    # # 创建一个模拟的视频特征序列 2 16 256
    # # 形状为 (batch_size, video_length, query_dim)
    # hidden_states = torch.randn(batch_size, video_length, query_dim)
    #
    # # 应用 TemporalAttention 2 16 256
    # output = temporal_attention(hidden_states)
    #
    # print("TemporalAttention 输出的形状:", output.shape)

    import torch

    # 假设 SparseCausalAttention 已经定义（如上文代码所示）
    ...
    # 创建 SparseCausalAttention 实例
    sparse_causal_attention = SparseCausalAttention(
        query_dim=query_dim,
        heads=num_heads,
        dim_head=dim_head,
        dropout=dropout,
    ...
    # 2 5 3 64 64
    hidden_states=torch.randn(32,3,query_dim)

    #video_length f  b f
    # 创建一个模拟的视频特征序列 32 1 256
    # 形状为 (batch_size * video_length, sequence_length, query_dim)
    # hidden_states = torch.randn(batch_size * video_length, 1, query_dim)

    # 应用 SparseCausalAttention 32 1 256
    output = sparse_causal_attention(hidden_states, video_length=video_length)

    print("SparseCausalAttention 输出的形状:", output.shape)
