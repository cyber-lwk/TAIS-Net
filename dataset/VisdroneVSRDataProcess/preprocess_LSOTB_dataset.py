...
def merge_train_val(train_path, val_path):
    """Merge the train and val datasets of visdrone.
...
def parse_args():
    ...
    parser.add_argument('-d','--root-path', type=str, help='root path for visdrone')
    parser.add_argument('-sr', '--save-root', type=str, help='root path for save visdrone sub dataset')
    parser.add_argument("-sp",'--split',nargs='*', default=['train','val','test'], help='split for Visdrone')
    parser.add_argument(
        '-s','--scales', nargs='*', default=[], help='scale factor list')
    #创建lmdb文件 可以加快计算效率 减少io读取时间
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    #     # dirpath = os.path.join(root_path,sp)
    #     train_path = osp.join(root_path, sp)               #..../train/
    ...
    generate_anno_file(root_path,file_name="meta_info_LSOTB_GT.txt")
