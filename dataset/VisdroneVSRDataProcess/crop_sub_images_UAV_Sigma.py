...
def worker(path, opt):
    ...
    img_name = str(f"{int(img_name):05d}")
    extension = ".png"
...
def imresize(img_path, output_path,up_path, scale=None, output_shape=None):
    ...
    #bicubic 上采样
    output_tensor = (output/255.0) * 2.0 -1.0
    output_tensor = torch.from_numpy(output_tensor).unsqueeze(0).permute(0,3,1,2)
    output_tensor = F.interpolate(output_tensor,scale_factor=4,mode='bicubic')
    output_tensor = output_tensor * 0.5 + 0.5
    output_tensor = output_tensor.clamp(0,1)
    output_tensor = output_tensor.squeeze().permute(1,2,0).numpy()*255
    cv2.imwrite(up_path,output_tensor.astype(np.uint8))
    ccc=0
...
def worker_down(clip_path, args):
    ...
    gt_dir = osp.join(args['input_folder'], 'GT', clip_path)
    bi_dir = osp.join(args['root_dir'], 'BIx4', clip_path)

    bdup_dir = osp.join(args['root_dir'], 'Bicubic4x', clip_path)
    # mmengine.utils.mkdir_or_exist(bi_dir)

    # mmengine.utils.mkdir_or_exist(bdup_dir)
    # imresize(osp.join(gt_dir, img), osp.join(bi_dir, img), scale=1 / 4)
    # bd_downsample(osp.join(gt_dir, img), osp.join(bd_dir, img))
    for sigma in args["sigmas"]:
        bd_dir = osp.join(args['root_dir'], f'BDx4_{sigma:.2f}', clip_path)
        mmengine.utils.mkdir_or_exist(bd_dir)
        img_list = sorted(os.listdir(gt_dir))
        for img in img_list:
            # imresize(osp.join(gt_dir, img), osp.join(bi_dir, img),osp.join(bdup_dir, img), scale=1 / 4)         #不带模糊的 直接下采样
            bd_downsample(osp.join(gt_dir, img), osp.join(bd_dir, img),sigma=sigma, scale=4)                 #带模糊的
...
def downsample_images(args):
    """Downsample images."""
    clip_list = []
    gt_dir = os.path.join(args['input_folder'],"GT")
...
def make_lmdb(data_path, lmdb_path, batch=5000, compress_level=0):
    ...
    for idx, (path, key) in enumerate(zip(img_path_list, keys)):
        ...
        print(f"read_path:{osp.join(data_path, path)}")
        img = mmcv.imread(osp.join(data_path, path))
        print(f"read_path:{osp.join(data_path, path)} img_shape:{img.shape}")
        h, w, c = img.shape
...
def main_extract_subimages(args):
    ...
    for sp in args.split:
        ...
        # generate_anno_file(opt['root_dir'],f'meta_info_LSOTB_{sp}.txt')
        # # create lmdb file
        # if args.make_lmdb:
...
def parse_args():
    ...
    # sigma
    parser.add_argument(
        '--sigmas', nargs='*', default=[1.0,1.3,1.6,1.9,2.2,2.5], help='scale factor list')
...
if __name__ == '__main__':
    ...
    args.sigmas = [float(v) for v in args.sigmas]
