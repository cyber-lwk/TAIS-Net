...
from pipeline.archs.mia_sliding_arch import SwinIRFM

"""
用于视频超分辨率的深度学习模型，它结合了掩码注意力机制和Transformer架构，
旨在提高低分辨率视频的分辨率，同时保持视频的时空一致性。
MIAVSR模型的主要目标是将低分辨率（LQ）视频序列提升为高分辨率（HR）视频序列。它通过以下几个关键模块实现这一目标：

    特征提取模块：从输入的低分辨率帧中提取特征。
    光流计算模块：使用SPyNet计算帧之间的光流，以便对特征进行对齐。
    特征传播模块：通过双向传播（前向和后向）对特征进行时空对齐和融合。
    上采样模块：将特征图上采样到高分辨率，并生成最终的高分辨率帧。
2. 关键模块
2.1 特征提取模块

    输入：低分辨率视频序列 lqs。
    操作：如果输入是低分辨率的，使用一个卷积层（conv_first）将输入帧的通道数从3（RGB）提升到嵌入维度（embed_dim）。
    输出：提取的特征图，用于后续的特征传播。

2.2 光流计算模块

    输入：低分辨率视频序列 lqs。
    操作：使用SPyNet计算相邻帧之间的光流。光流表示帧之间的像素运动，用于对齐特征图。
    输出：前向光流（flows_forward）和后向光流（flows_backward）。

2.3 特征传播模块

    输入：提取的特征图和计算的光流。
    操作：
        双向传播：模型通过双向传播（前向和后向）对特征进行对齐和融合。每个方向的传播包括两个阶段（backward_1, forward_1, backward_2, forward_2）。
        掩码注意力机制：在每个传播阶段，使用掩码注意力机制对特征进行时空融合，增强特征的表示能力。
        特征对齐：使用光流对齐特征图，确保特征在时空上的一致性。
    输出：经过时空融合的特征图。
...
2.4 上采样模块
    ...
    输入：经过时空融合的特征图。
    操作：
        使用卷积层（conv_before_upsample）将特征图的通道数调整为中间通道数（mid_channels）。
        使用像素重排（PixelShuffle）上采样特征图，提升分辨率。
        使用卷积层（conv_last）生成最终的高分辨率帧。
    输出：高分辨率视频序列。
...
MIAVSR模型的整体结构如下：
    ...
    特征提取：从输入的低分辨率帧中提取特征。
    光流计算：计算帧之间的光流，用于特征对齐。
    特征传播：
        后向传播1：从最后一帧开始，向后传播特征。
        前向传播1：从前一阶段的特征开始，向前传播特征。
        后向传播2：从第二阶段的特征开始，向后传播特征。
        前向传播2：从前一阶段的特征开始，向前传播特征。
    上采样：将特征图上采样到高分辨率，生成最终的高分辨率帧。
"""
...
class MIAVSR(ModelMixin,ConfigMixin,FromOriginalControlnetMixin):
    """MIA-VSR network structure.
    ...
    def __init__(self,
        ...
        # optical flow
        self.spynet = SpyNet(spynet_path)               #光流计算
        ...
        # 特征传播：patch_align模块，包含四个传播分支（backward_1, forward_1, backward_2, forward_2）
        ...
        # 上采样：conv_before_upsample、upconv1、upconv2、pixel_shuffle、conv_hr、conv_last
    ...
    def propagate(self, feats, flows, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0:
                ...
                cond = torch.stack([cond_n2, cond_n1, feat_current], dim=1)
                # patch alignment
            ...
            if i == 0:
                ...
                feat_prop, lastquary, predmask = self.patch_align[module_name](cond, lastquary, recurrent=False)
                pred.append(predmask)
    ...
    '''
    模型特点
        掩码注意力机制：通过掩码注意力机制，模型能够更好地处理帧内的局部特征和帧间的时空关系。
        双向传播：通过双向传播，模型能够充分利用前后帧的信息，增强特征的时空一致性。
        光流对齐：使用光流对齐特征图，确保特征在时空上的一致性。
        Transformer架构：结合Transformer架构，模型能够处理长序列的时空信息，提高超分辨率的效果。
    '''
    def forward(self, lqs):
        ...
        for iter_ in [1,2]:#[1, 2]
            for direction in ['backward', 'forward']:
                ...
                feats, pred = self.propagate(feats, flows, module)
