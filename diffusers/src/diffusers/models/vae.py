...
class IndDecoder(nn.Module):
    ...
    def forward(self, z, latent_embeds=None,num_scales: Optional[int] = 1,scale_factors=None):
        sample = z
        sample = self.conv_in(sample)
        lq_h,lq_w = sample.shape[-2:]
        upscale_dtype = next(iter(self.up_blocks.parameters())).dtype
        if self.training and self.gradient_checkpointing:
        ...
        # # 原始的
        # # sample = self.conv_out_conv(sample)
        # # 现在的
        ...
        # MACs, params = profile(self.conv_out, inputs=(sample, coord, cell, [h_new, w_new], return_img, 0,))  # 估计参数大小
        #
        # # 将结果转换为更易于阅读的格式
        # MACs, params = clever_format([MACs, params], '%.3f')
        #
        ...
        # # sample_out_LIIF = self.conv_out(sample, coord, cell, output_size=(h_new, w_new), return_img=return_img, bsize=0)
        ...
        w_in=w_in//4.0
        samples = []
        scales_list = scale_factors
        if scales_list is None:
            # scales_list = np.linspace(0, 2.0, num_scales + 1)
            scales_list = np.linspace(0, 4.0, num_scales + 1)
        ...
        for scale in scales_list:
            ...
            # sample_out = self.conv_out(sample, coord, cell, output_size=(h_new, w_new), return_img=return_img,bsize=4*4)
            # t2 = time.time()
